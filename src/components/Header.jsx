                ...
                <div className="container">
                    <div className="header-inner flex items-center justify-between lg:justify-start">
                        ...
                        <div className="header-actions flex items-center gap-3 md:gap-5">
                            {/*  @auth */}
                            {/* <div x-data="{ dropdownProfile: false }" className="profile relative">
                                <button @click="dropdownProfile = ! dropdownProfile"
                            className="flex items-center text-darkblue hover:text-blue transition lg: none">
                                <span className="sr-only">Профиль</span>
                                <img src="{{ auth()->user()?->avatar }}" className="shrink-0 w-7 md:w-9 h-7 md:h-9 rounded-full"
                                    alt="{{ auth()->user()?->name }}">
                                    <span className="hidden md:block ml-2 font-medium">{{ auth()-> user() ? -> name }}</span>
                                    <svg className="shrink-0 w-3 h-3 ml-2" xmlns="http://www.w3.org/2000/svg" fill="currentColor"
                                        viewBox="0 0 30 16">
                            ...
                            className="absolute z-50 top-0 right-10 xs:-right-10 sm:-right-36 w-[280px] sm:w-[300px] mt-14 p-4 rounded-lg shadow-xl bg-card">
                                <h5 className="text-body text-xs">Мой профиль</h5>
                                <div className="flex items-center mt-3">
                                    <img src="{{ auth()->user()?->avatar }}" className="w-11 h-11 rounded-full"
                                        alt="{{ auth()->user()?->name }}">
                                        <span className="ml-3 text-xs md:text-sm font-bold">
                                            {{ auth()-> user() ? -> name }}
                                            <span className="text-body text-xxs font-thin">
                                        ...
                                        <div className="mt-4">
                                            <ul className="space-y-2">
                                                <li>
                                                    <a href="#" className="text-body hover:text-white text-xs font-medium">
                                                        Мои заказы
                                                    </a>
                                                </li>
                                                <li>
                                                ...
                                                <li>
                                                    <a href="{{ route('filament.admin.pages.dashboard') }}" className="text-body hover:text-white text-xs font-medium">
                                        ...
                                        <div className="mt-6">
                                            <form action="{{ route('logout') }}" method="POST">
                                                ...
                                                <button type="submit" href="#"
                                                    className="inline-flex items-center text-body hover:text-gray">
                                                    <svg className="shrink-0 w-5 h-5" xmlns="http://www.w3.org/2000/svg" fill="currentColor"
                                                        viewBox="0 0 20 20">
                                                        <path
                                                    ...
                                                    <span className="ml-2 font-medium">Выйти</span>
                                                </button>
                                            </form>
                                        </div>
                                </div>
                            ...
                            <a href="{{ route('cart') }}" className="flex items-center gap-3 text-darkblue hover:text-gray">
                            ...
                            {/* @guest */}
                                <a href="{{ route('login') }}" className="profile hidden xs:flex items-center">
                                    ...
                                    <span className="profile-text relative ml-2 text-black text-xxs md:text-xs font-bold">Войти</span>
                                </a>
                            {/* @endguest */}
